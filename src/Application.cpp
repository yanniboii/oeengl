...
const string ASSETSPATH = "C:/Dev/oeengl/assets/";
const string SHADERPATH = "C:/Dev/oeengl/assets/Shaders/";
const string VERTEXPATH = "VertexShader.vert";
const string FRAGMENTPATH = "FragmentShader.frag";
const string OBJPATH = "Models/teapot.obj";
...
int main(void)
{
    ...
    Mesh objCube = fReader.Read((ASSETSPATH+OBJPATH),false, old);
    ...
    if (vStruct) {
        vector<Vertex> verts = objCube.GetVertices();
        vb = new VertexBuffer(verts.size() * sizeof(Vertex), &verts[0]);
    }
    else
    {
        vector<fvec3> verts = objCube.GetPos();
        vb = new VertexBuffer(verts.size() * sizeof(fvec3), &verts[0]);
    }
    /*vector<fvec3> vertPos;
    vertPos.reserve(verts.size());

    transform(
        verts.begin(), 
        verts.end(),
        back_inserter(vertPos),
        [](const Vertex& V) {return V.postion; });

    vector<fvec3> vertCol;
    vertCol.reserve(verts.size());

    transform(
        verts.begin(),
        verts.end(),
        back_inserter(vertCol),
        [](const Vertex& V) {return V.colors; });

    vector<fvec3> vertNorm;
    vertNorm.reserve(verts.size());

    transform(
        verts.begin(),
        verts.end(),
        back_inserter(vertNorm),
        [](const Vertex& V) {return V.normals; });

    vector<fvec2> vertUv;
    vertUv.reserve(verts.size());

    transform(
        verts.begin(),
        verts.end(),
        back_inserter(vertUv),
        [](const Vertex& V) {return V.uvs; });

    PrintMesh(verts, vertPos, vertNorm, vertCol, vertUv);*/
    ...
    vector<unsigned int> inds = objCube.GetFaces();

    IndexBuffer ib(&inds[0], inds.size());
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        if (false) {
            ...
            screenShader.Use();
            renderer.Draw(sva,sib,screenShader);
        }
